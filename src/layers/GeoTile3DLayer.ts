...
class GeoTile3DLayer extends Base3DLayer {
   ...
   */
    ...
    //console.debug(origin, forward, size, angle, sphereCenter, sphereRadius);
        ...
        const distanceClosestPoint =
      Math.cos(angle) * Math.sqrt(VlenSq - V1len * V1len) -
      V1len * Math.sin(angle);
    ...
    // loading chunks each 100 frames. Bad performance
        ...
        const positionWGS84: number[] = <number[]>(
      this.layerManager?.sceneViewer.positionWGS84()
    );
        const coordsWGS84: Coordinate = [ positionWGS84[0], positionWGS84[1] ];
        const coordsUtm: Coordinate = olProj.transform(
            coordsWGS84,
            "EPSG:4326",
            "EPSG:3857",
        );
        ...
        for (let i = -tiledistWalk; i <= tiledistWalk; i++) {
            for (let j = -tiledistWalk; j <= tiledistWalk; j++) {
                ...
                if (i * i + j * j > tiledistDraw * tiledistDraw) {
                    ...
                    const tileCenter = this.tileGrid.getTileCoordCenter([
                        tileCoords[0],
                        tileCoords[1] + i,
                        tileCoords[2] + j,
                    ]);
                    const tileCenterWGS84 = olProj.transform(
                        tileCenter,
                        "EPSG:3857",
                        "EPSG:4326",
                    );
                    const tileCenterScene =
            sceneViewer.projection.forward(tileCenterWGS84);
                    const sphereCenter = new Vector3(
                        tileCenterScene[0],
                        0,
                        tileCenterScene[1],
                    ); // TODO: Get median height from tile
   ...
   */
    ...
    //console.debug(tileCoords);
        ...
        const tileExtentMinScene =
      this.layerManager!.sceneViewer.projection.forward(
          olProj.transform(
              extent.getBottomLeft(tileExtent),
              "EPSG:3857",
              "EPSG:4326",
          ),
      );
        const tileExtentMaxScene =
      this.layerManager!.sceneViewer.projection.forward(
          olProj.transform(
              extent.getTopRight(tileExtent),
              "EPSG:3857",
              "EPSG:4326",
          ),
      );
        ...
        const tileUrlBase =
      this.layerManager!.sceneViewer.viewerState.dddConfig.tileUrlBase;
        const tileUrlSuffix =
      this.layerManager!.sceneViewer.viewerState.dddConfig.tileUrlSuffix; // e.g. ".uncompressed"

        //const tileUrl = tileUrlBase + z + "/" + x + "/" + y + ".glb";
        const tileUrl =
      tileUrlBase + z + "/" + x + "/" + y + tileUrlSuffix + ".glb";
        ...
        const pivot = new Mesh(
            "chunk_" + tileKey.replaceAll("/", "_"),
      this.layerManager!.sceneViewer.scene,
        ); // new Mesh("chunk_" + tileKey, this.layerManager.sceneViewer.scene);
    ...
    this.layerManager!.sceneViewer.queueLoader.enqueueLoadModel(
        tileUrl,
        // onSuccess
        ...
        // Ensure tile still exists:
            ...
            this.layerManager!.sceneViewer.shadowGenerator.getShadowMap()!.renderList!.push(
                mesh,
            );
                }
                ...
                if (heightMax > maxHeight && heightMax !== 0) {
                    maxHeight = heightMax;
                }
            ...
            const tileCenterScene =
          this.layerManager!.sceneViewer.projection.forward(tileCenterWGS84);
            ...
            if (!this._initialHeightSet) {
                ...
                const ray = new Ray(
                    new Vector3(
              this.layerManager!.sceneViewer.camera!.position.x,
              -100.0,
              this.layerManager!.sceneViewer.camera!.position.z,
                    ),
                    new Vector3(0, 1, 0),
                    3000.0,
                );
                const pickResult =
            this.layerManager!.sceneViewer.scene.pickWithRay(ray);
                if (
                    pickResult &&
            ...
            this.layerManager!.sceneViewer.camera!.position.y =
              pickResult.distance - 100.0;
            if (
              this.layerManager!.sceneViewer.viewerState.positionGroundHeight
            ) {
              this.layerManager!.sceneViewer.camera!.position.y +=
                this.layerManager!.sceneViewer.viewerState.positionGroundHeight;
            } else {
        ...
        this.layerManager!.sceneViewer.scene.blockfreeActiveMeshesAndRenderingGroups =
          true;
        ...
        if (this.layerManager!.sceneViewer.viewerState.sceneSelectedMeshId) {
            const criteria = {
                _node_name:
              this.layerManager!.sceneViewer.viewerState.sceneSelectedMeshId,
            };
            //console.debug(criteria);
            const foundMesh = this.layerManager!.sceneViewer.findNode(
            <Mesh>pivot,
            criteria,
            );
        ...
        // eslint-disable-next-line no-console
            ...
            const request = ex.innerError.request;
            ...
            this.layerManager!.sceneViewer.viewerState.remoteQueueJobsStatus.push(
                responseData,
            );
                } catch (e) {
    ...
    loadQuadMarker(tileCoords: number[], color: Color3 = Color3.Gray()): Node {
      ...
      this.layerManager!.sceneViewer.scene,
        ...
        const materialPlane = new StandardMaterial(
            "textureTile_" + tileKey,
      this.layerManager!.sceneViewer.scene,
        );
    ...
    groundTextureLayerProcessNode(tile: GeoTile3D, node: Node): void {
        ...
        this.layerManager!.sceneViewer.projection.forward(
            ...
            const sizeWidth = Math.abs(tileExtentMaxScene[0] - tileExtentMinScene[0]);
            const sizeHeight = Math.abs(
                tileExtentMaxScene[1] - tileExtentMinScene[1],
            );

            // Create material
            //console.debug("Creating material for ground texture: " + url);
            const tileKey = tileCoords[0] + "/" + tileCoords[1] + "/" + tileCoords[2];
            const url = this.replaceTileCoordsUrl(
                tileCoords,
                this.groundTextureLayerUrl,
            );
            materialGround = new StandardMaterial(
                "materialGround_" + tileKey,
        this.layerManager!.sceneViewer.scene,
            );
            ...
            const materialGroundTexture: Texture = new Texture(
                url,
        this.layerManager!.sceneViewer.scene,
            );
        ...
        mesh.metadata.gltf.extras
                ...
                if (
                    metadata["ddd:path"].indexOf("/Areas") > 0 ||
